...
public class CharacterInterface : MonoBehaviour
{
    ...
    void Update()
    {
        //When the path is created start moving the character along the way
        if (character.Path.Count > 0)
        {
            //If we've reached the next tile grab the next one
            if (Vector3.Distance(character.transform.position, character.TargetTile.transform.position) < 0.001f)
            {
                Approach();
            }
            float step = moveTime * Time.deltaTime;
            //Face the next tile
            Vector3 direction = character.TargetTile.transform.position - character.transform.position;
            character.transform.rotation = Quaternion.LookRotation(transform.forward,direction);
            //Move toward the next tile
            Vector3 newDir = Vector3.MoveTowards(character.transform.position, character.TargetTile.transform.position, step);
            character.transform.position = newDir;
        }
        // Run the core coroutine.
        StartCoroutine(HandleInput());
    }

    //Tries to set the next current and target tile for the character
    void Approach()
    {
        try
        {
            //Current tile is the one we're on
            character.CurrentTile = character.Path.Pop().Tile;
            //Target tile is the next tile to move to
            character.TargetTile = character.Path.Peek().Tile;
        }
        catch
        {
