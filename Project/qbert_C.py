...
class DQN(nn.Module):
    ...
    def __init__(self, n_observations, n_actions):
        ...
        self.layer1 = nn.Conv2d(3,32,(8,8),4)
        self.layer2 = nn.MaxPool2d(2,2)
        self.layer3 = nn.Conv2d(32,64,(4,4),2)
        self.layer4 = nn.MaxPool2d(2,2)
        self.layer5 = nn.Flatten()
        self.layer6 = nn.Linear(1280, 128)
        self.layer7 = nn.Linear(128, n_actions)
...
def train_model():
    ...
    for i_episode in range(num_episodes):
        ...
        reward = 0 #⚠️
        terminated = False #⚠️
        observation = env.reset()[0] #⚠️
        # Initialize the environment and get it's state
        state, info = env.reset()
        state = torch.tensor(state, dtype=torch.float32, device=device).unsqueeze(0)
        last_life = info.get("lives")
        last_action = 0
        agent = Agent(env.action_space) #⚠️
        for t in count():
            action = torch.tensor([[agent.act(observation, reward, terminated)]], device=device) #⚠️
            observation, reward, terminated, truncated, _ = env.step(action) #⚠️
